...
- **Chosen — plaintext in the private repo, with a built-in opt-out**: crate prompts for any token it can't find and offers to save it synced or local-only, so gitignoring `mcp-tokens.env` turns the per-machine mode on with zero extra code.
